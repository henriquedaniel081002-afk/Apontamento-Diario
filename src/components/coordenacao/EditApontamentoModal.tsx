...
export const EditApontamentoModal: React.FC<Props> = ({ apontamento, isOpen, onClose, onSave, contextLabel = 'Edição de apontamento' }) => {
  ...
  return <ModalShell isOpen={isOpen} onClose={onClose} title="Editar apontamento" description={`${contextLabel} • ${apontamento.setor} • Responsável: ${apontamento.userName}`} size="xl" busy={saving} footer={<div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="max-w-2xl text-xs leading-relaxed text-slate-500">{isImported ? 'Produção e data vieram do Excel e ficam bloqueadas. As ocorrências podem ser corrigidas sem alterar a produção importada.' : 'Setor, responsável e tipo de bobina originais são preservados.'}</p><div className="grid w-full grid-cols-1 gap-2 min-[380px]:grid-cols-2 sm:w-auto sm:flex sm:shrink-0"><Button variant="secondary" onClick={onClose} disabled={saving}>Cancelar</Button><Button onClick={() => void handleSave()} isLoading={saving} loadingLabel="Salvando…" leftIcon={<Save className="size-4" />}>Salvar alterações</Button></div></div>}>

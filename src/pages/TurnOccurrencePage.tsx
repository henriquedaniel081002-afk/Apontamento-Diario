...
export const TurnOccurrencePage: React.FC<Props> = ({ user, onNavigateToHistory }) => {
  ...
  useEffect(() => {
    if (!registrationOpen || !selectedTurn) return;
    let active = true;
    ...
    const load = async () => {
      ...
      setSaveError(null);
      try {
        const existing = await apontamentoService.getByDateAndSector(selectedDate, String(user.setor || ''), user.id);
        if (!active) return;
        setRecord(existing);
    ...
    void load();
    return () => { active = false; };
  }, [registrationOpen, selectedDate, selectedTurn, user.id, user.setor]);
  ...
  const handleSave = async () => {
    if (!selectedTurn || isSaving || (user.setor !== 'PINTURA' && user.setor !== 'SOLDA')) return;
    setIsSaving(true);
    setSaveError(null);
    try {
      ...
      setToast({
        ...
        type: 'success',
        message: `${finishedTurn} registrado com sucesso em ${formatDateBR(selectedDate)}. ${updated.producoes.length ? 'A produção já está vinculada ao apontamento.' : 'As ocorrências ficarão salvas até a produção ser importada.'}`,
    ...
    return (
      <PageContainer className="app-page space-y-5 py-6 sm:py-8">
        <PageHeader
          ...
          description="Selecione o turno antes de iniciar. Em Solda e Pintura, as ocorrências podem ser registradas mesmo antes da importação da produção."
          metadata={<Badge variant="neutral">{user.setor}</Badge>}
        />
        ...
        <Surface tone="base" padding="md" className="space-y-5">
          ...
          <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-muted)] p-4">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--text-tertiary)]">2. Registrar</p>
                <p className="mt-1 text-sm font-bold text-[var(--text-primary)]">{selectedTurn ? `${selectedTurn} pronto para registrar` : 'Selecione um turno para continuar'}</p>
              </div>
              <Button disabled={!selectedTurn} leftIcon={<ClipboardPlus className="h-4 w-4" />} onClick={openRegistration}>
  ...
  return (
    <PageContainer className="app-page space-y-5 py-6 sm:py-8" aria-busy={loadingRecord || isSaving || undefined}>
      <PageHeader
        ...
        metadata={<Badge variant="info">{selectedTurn}</Badge>}
        actions={<Button variant="secondary" leftIcon={<ArrowLeft className="h-4 w-4" />} disabled={isSaving} onClick={closeRegistration}>Trocar turno</Button>}
      />
      ...
      <Surface tone="base" padding="md" className="grid gap-4 sm:grid-cols-[minmax(0,18rem)_1fr] sm:items-end">
        ...
        <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-muted)] px-4 py-3">
          ...
          <p className="mt-1 text-xs text-[var(--text-tertiary)]">Se já houver ocorrências neste turno/data, elas serão carregadas para conferência e atualização.</p>
        </div>
      </Surface>
      ...
      {loadingRecord ? (
        ...
        <>
          ...
          <div>
            <div hidden={currentStep !== 1}><ParadasFaltaMaterialSection key={`material-${selectedDate}-${selectedTurn}-${draftResetVersion}`} itens={paradasFaltaMaterial} onAdd={(item) => setParadasFaltaMaterial((current) => [...current, item])} onUpdate={(item) => setParadasFaltaMaterial((current) => current.map((value) => value.id === item.id ? item : value))} onDelete={(id) => setParadasFaltaMaterial((current) => current.filter((value) => value.id !== id))} /></div>
            <div hidden={currentStep !== 2}><ParadasMaquinaSection key={`maquina-${selectedDate}-${selectedTurn}-${draftResetVersion}`} itens={paradasMaquina} onAdd={(item) => setParadasMaquina((current) => [...current, item])} onUpdate={(item) => setParadasMaquina((current) => current.map((value) => value.id === item.id ? item : value))} onDelete={(id) => setParadasMaquina((current) => current.filter((value) => value.id !== id))} /></div>
            <div hidden={currentStep !== 3}><NaoConformidadesSection key={`nc-${selectedDate}-${selectedTurn}-${draftResetVersion}`} itens={naoConformidades} onAdd={(item) => setNaoConformidades((current) => [...current, item])} onUpdate={(item) => setNaoConformidades((current) => current.map((value) => value.id === item.id ? item : value))} onDelete={(id) => setNaoConformidades((current) => current.filter((value) => value.id !== id))} /></div>
            <div hidden={currentStep !== 4}><FaltasSection key={`faltas-${selectedDate}-${selectedTurn}-${draftResetVersion}`} faltas={faltas} onAdd={(item) => setFaltas((current) => [...current, item])} onUpdate={(item) => setFaltas((current) => current.map((value) => value.id === item.id ? item : value))} onDelete={(id) => setFaltas((current) => current.filter((value) => value.id !== id))} /></div>
            <div hidden={currentStep !== 5}><ObservacoesSection key={`obs-${selectedDate}-${selectedTurn}-${draftResetVersion}`} observacoes={observacoes} onAdd={(item) => setObservacoes((current) => [...current, item])} onUpdate={(item) => setObservacoes((current) => current.map((value) => value.id === item.id ? item : value))} onDelete={(id) => setObservacoes((current) => current.filter((value) => value.id !== id))} /></div>
          ...
          <div className="sticky-action-bar sticky bottom-3 z-20 rounded-2xl border p-3 backdrop-blur-2xl sm:p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm font-semibold text-[var(--text-primary)]">{selectedTurn} · Etapa {currentStep} de 6 · {stepLabels[currentStep - 1]}</p>

...
describe('estrutura da exportação Excel', () => {
  it('mantém as três coleções e inclui justificativas na aba de observações', () => {
    ...
    expect(workbook.produzido).toEqual([
      { data: '13/ago', potencia: 112.5, qtde: 8, setor: 'BOBINA AT', linha: 'MON' },
    ]);
    expect(workbook.faltas).toEqual([
      { data: '13/ago', faltas: 1, turno: 2, setor: 'BOBINA AT', linha: 'TRI' },
    ]);
    expect(workbook.obs).toEqual([
      { data: '13/ago', obs: 'Parada programada', setor: 'BOBINA AT', linha: 'MON' },
      { data: '13/ago', obs: 'Atestado médico', setor: 'BOBINA AT', linha: 'TRI' },
    ]);
  });
